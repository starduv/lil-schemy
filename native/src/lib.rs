...
fn generate_schemas(mut cx: FunctionContext) -> Result<Handle<JsObject>, Throw> {
    ...
    let ast_map = serde_json::from_str(&options_handle.get::<JsString, _, _>(&mut cx, "asts")?.value(&mut cx))
        .expect("Could not parse asts");
    let module_map = serde_json::from_str(&options_handle.get::<JsString, _, _>(&mut cx, "modules")?.value(&mut cx))
        .expect("Could not parse module map");
